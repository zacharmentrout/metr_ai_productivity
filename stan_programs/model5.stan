...
//
// Identification: comfort coefficient = -1 in forecast, task_burden coefficient = +1
...
parameters {
  ...
  // Coefficients
  real beta_burden;         // Burden -> completion
  real beta_trt;            // Treatment effect
  real beta_t_c;            // Comfort -> completion
  real lambda_e;            // Comfort -> exposure
  real lambda_r_c;          // Comfort -> resources
  real lambda_r_b;          // Burden -> resources
...
model {
  ...
  beta_t_c ~ normal(-1, 0.6);
  lambda_e ~ normal(1, 0.43);
  lambda_r_c ~ normal(-1, 0.43);
  lambda_r_b ~ normal(1, 0.43);
  ...
  // Likelihood for forecasts (identification: -comfort + burden)
  {
    vector[N] mu_f;
    for (n in 1:N) {
      mu_f[n] = exp(alpha_f - comfort[dev_idx[n]] + task_burden[n]);
    }
  ...
  for (i in 1:N_obs_resources) {
    ...
    real gamma_r = lambda_r_c * comfort[dev_idx[n]] + lambda_r_b * task_burden[n];
  ...
  // Likelihood for completion times
  {
    vector[N] mu_t;
    for (n in 1:N) {
      mu_t[n] = alpha_t + beta_t_c * comfort[dev_idx[n]] + beta_burden * task_burden[n]
...
generated quantities {
  ...
  for (n in 1:N) {
    real mu_t_n = alpha_t + beta_t_c * comfort[dev_idx[n]] + beta_burden * task_burden[n]
                  + beta_trt * ai_access[n];
  ...
  for (n in 1:N) {
    real mu_f_n = exp(alpha_f - comfort[dev_idx[n]] + task_burden[n]);
